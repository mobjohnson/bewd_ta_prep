...
# 1) Reference:
n = mine[:digs].length
puts "Total number of dig sites: #{ n }"


# 2) Reference:
name = mine[:digs][1][:name]
puts "Name of the second dig site: #{ name }"


# 3) Reference:
name = mine[:donors][0]['name']
puts "Name of the first donor: #{ name }"


# 4) Solve programatically:
dig = mine[:digs].find {|dig| dig[:id] == 7}
puts "Age of the dig with an id of 7: #{ dig[:age] }"


# 5) Solve programatically:
digs = mine[:digs].select {|dig| dig[:is_active]}
puts "Number of active digs: #{ digs.length }"


# 6) Solve programatically:
total = 0
finds = mine[:digs].each do |dig|
